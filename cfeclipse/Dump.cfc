...
- classic: classic view with html/css/javascript"},
	...
	function openInEditor(file,line) {
		...
		var url='#getContextRoot()#/cfeclipsecall.cfm?cfe=' + file + '|' + line;
			oXmlHttp=GetHttpObject(stateChanged)
			oXmlHttp.open('GET', url , true)
			oXmlHttp.send(null)
	}
	function stateChanged() {if (oXmlHttp.readyState==4 || oXmlHttp.readyState=='complete') {}}
	...
	function GetHttpObject(handler) {
		try {
			...
			var linked = rereplaceNoCase(result,"\((/[^/]*/+\S+)\:(\d+)\)",'<a href="" onclick="openInEditor(''\1'',\2); return false;">\1:\2</a>)',"all");
			echo('<div id="#dumpID#">#linked#</div>' & NEWLINE);
